      ...
      <div className='d-flex h-100'>
        <div className='d-flex-col tabs tab-seperator'>
          {settings.map(({ id, label }) => (
            <SettingRackTab key={id} id={id} onChangeTab={setActiveTab} selected={activeTab === id} label={label} />
          ))}
        </div>

        <div className='flex-1 setting-area h-100' style={{ height: props.height, width: props.width }}>
          {/* {
                        activeTab === 0 && <Overview></Overview>
                    } */}
          {activeTab === 1 && <Amp dims={{ height: props.height, width: props.width }}></Amp>}
          {/* {
                        activeTab === 2 && <Additive dims={{ height, width }} partials={props.activeOscillator.partials} updateOscData={props.updateOscData}></Additive>
                    } */}
        </div>
      </div>
    </div>
